...
**`year` 的口径是「历史视图锚点年」，不是「唯一发明年」**（2026-09-16 修订）。很多概念本就没有诞生日——
CISC 是 RISC 出现后的回溯命名，KV-Cache 是 Transformer 解码的实现产物，编译器/操作系统的"第一个"取决于怎么定义。
这类节点的处理办法是**挑一个代表作当锚点，并在正文写明依据和其他说法**：
...
**仍然不许填猜测值**：锚点必须能指出具体的论文 / 产品 / 公认叙述，写不出依据就继续留空。
区别在于"有争议"和"不知道"——有争议的挑一个并说清楚，不知道的留空。

> 2026-09-16 之前的规则是"未知年份留空，不填写猜测值"，结果是 CISC / 编译器 / JIT / OS / ROM 五个节点
> 长期挂着「核实 year」待办、进不了历史视图，沿演化链导览也就无路可走。改成锚点口径后这些节点全部落位。
