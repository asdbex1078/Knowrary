...
> **这是一个案例，不是这个程序的一部分。** 它原本长在 Knowrary 仓库里，
> 2026-09-22 程序与知识库分家之后搬去了知识库那边（数据归数据），
> 这里只留下**脚本和定时任务提示词**当样板：一个每天自己跑、把外部信息收进你的库、
> 每周汇成一份周报的小系统长什么样。
>
> 留下的是 `scripts/` 和 `cron/`；`pool/`、`digests/`、`state.json` 是它跑起来自己生成的，
> 那些是**你的数据**，属于知识库，不属于程序仓库。想用就整个目录拷到你自己的库里，
> 改掉 `publish.sh` 里的仓库地址和 `cron/` 里的路径。
...
- `pool/` 每日收集池（JSON 供机器读，MD 供人读）——**跑起来才有，不在这个案例里**
- `digests/` 每周周报；`digests/raw/` 周报合并材料——同上
...
- `state.json` 去重 / 版本比对状态（45 天去重窗口）——同上
