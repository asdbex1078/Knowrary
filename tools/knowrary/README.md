...
vault **不是本仓库**（2026-09-22 程序与知识库分家，程序目录当库会被直接拒）。
`--vault` 不给就按「环境变量 `KNOWRARY_VAULT` → 设置页选中的库（`~/.knowrary/config.json`）」解析，
所以下面的命令一律不写死库路径。解析器只扫库里的 `nodes/` 和 `fields/`。

零第三方依赖（Python ≥ 3.10）。规范见 `doc/规范文档/Markdown文档规范.md`。

下面的路径都以**仓库根目录**为基准（`cd` 到仓库根再敲）：

```bash
KG=tools/knowrary/knowrary.py

# 1. 旧 vault（- 类型 → [[目标]]）→ 第二版结构
python3 $KG vault <旧 vault> <新目录> --field 计算机体系结构   # 已于 2026-09-10 执行过一次，产物就是现在那份正式库
...
python3 $KG layout init  [--vault <vault>] [--force]   # 按 field / nodes 子目录两级分组 + 组内网格
python3 $KG layout check [--vault <vault>]             # 孤立记录、Inbox 统计
#    服务启动时若无 layout.json 会自动生成同样的初始布局

# 2b. 校验（错误退出码 1）——与 index 共用同一套解析和诊断，额外查密钥泄露与索引契约
python3 $KG check [<vault>]

# 2c. 自测（覆盖幂等、诊断、归一、契约）。用 .venv：有两条用例跨到服务层，要 pydantic v2
.venv/bin/python tools/knowrary/tests/run.py [关键字]
...
# 4. LLM 配置：~/.knowrary/llm.local.json——**用户级，所有库共用**（KNOWRARY_LLM_CONFIG 可改路径）
#    模板 ~/.knowrary/llm.example.json，第一次运行时从仓库 seed/user/ 铺过去
...
| `tests/run.py` | 自测（142 个用例）。core 本身零依赖，但用例里有两条跨到服务层，所以要用 `.venv` 跑 |
