...
def timeless_是看过了本来就没年份_和还没填要分开():
    ...
    实盘上「缺 year」挂着 49 个，其中只有 9 个真有历史锚点：
    其余是结构概念（寄存器 / 栈内存 / 控制器）、外部学科入口（数论 / 物理-电学）
    和工程原则。它们不是"某一年发生的事"，硬填一个年份就是编。

    所以要给「看过了，它本来就没有年份」一个说法，别和「还没填」混在一起。
    """
    d = _digest_of({
        "nodes/x/有年份.md": node_md("有年份", extra="year: 1990\n"),
        "nodes/x/还没填.md": node_md("还没填"),
        "nodes/x/本来就没有.md": node_md("本来就没有", extra="timeless: true\n"),
    })
    assert d["no_year"] == ["还没填"], d["no_year"]
    assert d["counts"]["no_year"] == 1, d["counts"]
