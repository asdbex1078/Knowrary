...
**同一个坑前端也踩过**（2026-09-20 修）：热力图每一格的 key 用了
`new Date().toISOString().slice(0, 10)`，在 UTC+8 整张图往前错一天，
今天那格顶着昨天的 key 去查、于是永远查不到——界面上的样子是
"标题写着 1 天有动静、格子一个都不亮"。`learned` / `placedAt` 也是同一个写法，
错的日期还会被永久写进 md。现在统一走 `web/src/today.js`。
**"今天"要按谁的时区算，是这套数据里最容易两边各写一遍的东西。**
"""
