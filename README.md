...
> **愿景：沉淀一座属于自己的、活的知识库**——看得见（可视化）、长得大（可成长）、
> 还能把别人的脑子抄过来（跨库复制知识点）。

它能做什么：

1. **跟着 AI 学新东西**：和教练对话，把学到的东西直接沉淀成节点，相当于把 AI 蒸馏进自己的库。
   代价是消耗你自己的 token。
2. **学习者 + 审核者两个角色**：学习者（`learn`）负责教，打开「写入审核」后，审核者（`review`）
   会在内容写入前校验知识点对不对。审核同样要烧 token。
   - 学习者建议配强模型，审核者弱一点没关系。
   - **最佳实践：两个角色别用同一家族的模型。** 同一家族的预训练语料大体相同，错会错到一块去，
     互审就看不出问题。
3. **整理、导入自己的老笔记**：把已有的笔记和文章拆成节点收进库里。
   > ⚠️ **注意**：笔记入库时，程序会在正文前面加上元数据（frontmatter），在后面追加关系等内容，
   > 不再是原样的纯文本。
4. 更多功能见下文「[常用命令](#常用命令)」。

**Knowrary = know + library**：一个在本机跑的个人知识图谱程序。你的笔记是一堆普通的 Markdown，
它把这些 md 渲染成可拖动的结构图，再往上，增加出题与复习、把文章拆成节点、跟教练对话这些事。
...
#   写入审核（设置 →「写入审核」，默认关）：往 md 里写正文之前先让 review 角色看一眼对不对
#     **关着也照查一层**：未登记的关系类型、正文里指不到的 [[链接]]、正文太薄、新点零关系、名字撞车——
#     这些是算出来的，不花钱不用等，照常显示在卡上，只是不拦路
#     开了之后只审正文类写入（新建 / 补正文 / 重写正文），加边改年份不审；挡下时卡上给依据和改法，
#     「仍然写入」是留的后门（可在设置里关），每次强制都记进 .knowrary/issues.jsonl
...
#   历史视图 tab：X 轴年份、泳道按所选时间线的子分组分（也可按抽象层 / 按流派），被激活关系是金色流动虚线，
#     滑块按年回放（不改结构布局）；勾「主干道」把最长的那条演化链拉成一条水平主轴、旁支挂上下
#     （挑不出链就退回泳道）；播放条上的「导览」沿这条链一站一站走，镜头推到节点上讲解
#     **双击一个点 = 只看它那条演化线**：祖先 + 后代 + 直接旁系（同父兄弟、同子汇流），
#     旁系画淡；`被激活` 不算血缘（跨代点燃会把整族拉进来）。Esc 退出
