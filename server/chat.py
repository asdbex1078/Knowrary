...
画布卡片上只有这一句，所以短；`body` 是**笔记层**，落盘时整篇保留，**一律写足**。
项目的难度档只管聊天讲多深、出题考多深，**不管正文记多满**——「只要了解」的点，
半年后回看照样要看得懂它是什么、为什么、长什么样。聊天口气短，正文不跟着短。

**这次没聊到的节，用你自己知道的补足，段首标「（补充）」**，让我分得清哪些是聊出来的、
哪些是你补的。补的只能是你有把握的事实；没把握的宁可不写，也别编。
审校会核事实，所以补的年份、人名、数字要经得起查。
唯一的例外是「我的理解」：那一节只写我的话，我理解到哪写到哪，没懂就写「没懂：…」，
**不许替我补**。**事实层不留白**——人物、学科、年份、出处这些我口述里说过的词，
一个都不能在入库时被精简掉，少一个词以后就少一条能连的边
（「神经生理学家 + 数学家」丢了，神经元和神经网络数学那两条线就接不上）。

新建节点的 `body` 按这个骨架写，**每一节都要有**（「容易搞混的」确实没有相近概念时可以省）：
...
## 怎么运作
关键机制，能画就画（代码块 / 步骤 / 简图）。数字、边界条件、原始出处（论文 / 年份 / 人物）能给就给。

## 例子
**至少一个具体案例**：一段代码、一次真实的调用过程、一个历史事件、一组算出来的数字——
能让我照着走一遍的那种，不是「比如在很多场景下都有用」。能举反例（它失效的情况）更好。
...
**「线头」是给以后连边留的钩子，不能省。** 一行一个：口述里出现、这次没建成节点的
